  ...
  async addDayDetails(cobbdto: CobbDailyDto): Promise<any> {
    const datadetails = await this.prisma.cobbDaily.create({
      data: {
        ...
        male_feed_total_kgm:
          (cobbdto.male_feed_gms_per_bird * cobbdto.total_number_male) / 1000,
        female_feed_gms_per_bird: cobbdto.female_feed_gms_per_bird,
        female_feed_total_kgm:
          (cobbdto.female_feed_gms_per_bird * cobbdto.total_number_female) /
          1000,
